# Python 72 / 139
- [x] Basics 19 / 19
- - [x] Simple programs 10 / 10
   - - [x] Introduction to Python
   - - [x] Overview of the basic program
   - - [x] Multi-line programs
   - - [x] Program execution
   - - [x] Introduction to Python shell
   - - [x] Taking input
   - - [x] Program with numbers
   - - [x] Experiments with Python shell
   - - [x] Command line arguments
   - - [x]  Argparse module<br><br>
  
  
- - [x] Modules and packages 5 / 5 
   - - [x] Load module
   - - [x] 10
   - - [x] In project
   - - [x] Create module
   - - [x] Packages
   - - [x] How to read the documentation
   - - [x] Pip <br><br>

- - [x] Builtins 4 / 4
   - - [x] Sep and end arguments of print
   - - [x] Any and all
   - - [x] Map and filter
   - - [x] Flush and file arguments of print<br><br>

-----------------------------------------------------------------------------------------------

- [ ] Code quality 10 / 16
- - [x] Code style 5 / 5
   - - [x] PEP 8
   - - [x] Comments
   - - [x] Naming variables
   - - [x] Docstrings
   - - [x] Avoiding bad comments <br><br>
   
- - [ ] Errors and exceptions 5 / 5
   - - [x] Errors
   - - [x] Built-in exceptions
   - - [x] Exceptions
   - - [x] Exception handling
   - - [x] User-defined exceptions<br><br>

- - [ ] Testing and debugging 0 / 6
   - - [ ] Debugging in shell
   - - [ ] Assert statement
   - - [ ] Testing user input
   - - [ ] Unit testing in Python
   - - [ ] How to read a traceback
   - - [ ] Unittest in more detail <br><br>
 -----------------------------------------------------------------------------------------------

- [ ] Working with data 23 / 45
- - [ ] Data types and operations 11 / 19
   - - [x]  Basic data types and operations 4/4
   - - - [x] Basic data types
   - - - [x] Integer arithmetic
   - - - [x] Type casting
   - - - [x] Comparisons <br><br>
   
   - - [ ]  Strings 4 / 6
   - - - [x] Quotes and multi-line strings
   - - - [x] String formatting
   - - - [x] Basic string methods
   - - - [ ] Split and join
   - - - [x] Search in a string
   - - - [ ] Escape sequences<br><br>
  
   - - [ ]  Objects in Python 1 / 5
   - - - [x] Variables
   - - - [ ] Identity testing
   - - - [ ] Copy of an object
   - - - [ ] Hashable
   - - - [ ] Objects in Python <br><br>

   - - [x]  Boolean 2 / 2
   - - - [x] Boolean logic
   - - - [x] Conversion to boolean <br><br>

   - - [ ]  Bytes 0 / 2 
   - - - [ ] Bytes basics
   - - - [ ] Creating bytes <br><br>



- - [ ] Collections 10 / 17
   - - [ ]  Lists 3 / 7
   - - - [x] List
   - - - [x] Indexes
   - - - [ ] Slicing
   - - - [x] Operations with list
   - - - [x] List comprehension
   - - - [ ] Nested lists
   - - - [ ] Sorting a list<br><br>


   - - [x]  Dictionaries 3 / 3
   - - - [x] Dictionary
   - - - [x] Dictionary methods
   - - - [x] Operations with dictionary <br><br>

   - - [x]  Sets 2 / 2
   - - - [x] Set
   - - - [x] Set operations <br><br>

   - - [ ]  Tuples 1 / 2
   - - - [x] Tuple
   - - - [ ] Operations with tuple <br><br>

- - [ ] Working with files 2 / 9
   - - - [ ] Shelves
   - - - [x] Files in Python
   - - - [x] Writing files
   - - - [ ] Working with CSV
   - - - [ ] Pickle
   - - - [ ] os module
   - - - [ ] Openpyxl
   - - - [ ] Reading files
   - - - [ ] Context manager


-----------------------------------------------------------------------------------------------
- [ ] Control flow 11 / 20
- - [ ] Functions 5 / 11

   - - [x]  Invoking a function
   - - [x]  Declaring a function
   - - [x]  Arguments
   - - [x]  Lambda functions
   - - [ ]  Default arguments
   - - [ ]  Args
   - - [ ]  Kwargs
   - - [ ]  Recursion in Python
   - - [x]  Scopes
   - - [ ]  Function decorators
   - - [ ]  Functional decomposition <br><br>

- - [x] Control flow statements  6 / 6

   - - [x]  If statement
   - - [x]  Else statement
   - - [x]  Elif statement
   - - [x]  While loop
   - - [x]  For loop
   - - [x]  Loop control statements <br><br>

- - [ ] Iterators and generators 0 / 3

   - - [ ]  Custom generators
   - - [ ]  Iterators
   - - [ ]  Itertools module <br><br>

-----------------------------------------------------------------------------------------------
- [ ] Additional instruments 2 / 27
- - [ ] Math 2 / 5
   - - [x]  Math functions
   - - [x]  Random module
   - - [ ]  Float special values
   - - [ ]  Statistics module
   - - [ ]  SciPy overview <br><br>

- - [ ] Time 0 / 3
   - - [ ]  Datetime module
   - - [ ]  Datetime parsing and formatting
   - - [ ]  Time module <br><br>

- - [ ] Regular expressions 0 / 7
   - - [ ]  Regexps in Python
   - - [ ]  Escaping in regexps
   - - [ ]  Groups and alternations
   - - [ ]  Match object and flags
   - - [ ]  Regexp sets and ranges
   - - [ ]  Shorthands
   - - [ ]  Regexp quantifiers<br><br>

- - [ ] Networking 0 / 7
   - - [ ]  Requests: retrieving data
   - - [ ]  Requests: manipulating data
   - - [ ]  Json module
   - - [ ]  XML in Python
   - - [ ]  BeautifulSoup: working with HTML
   - - [ ]  BeautifulSoup: working with XML
   - - [ ]  Socket module <br><br>

- - [ ] Coding environments 0 / 3
   - - [ ]  The Jupyter Notebook
   - - [ ]  Virtual Environment
   - - [ ]  Google Colab<br><br>

- - [ ] SQL Alchemy 0 / 2
   - - [ ]  Intro to SQL Alchemy
   - - [ ]  QL Alchemy Mappings <br><br>

-----------------------------------------------------------------------------------------------
- [ ] Object-oriented programming 7 / 12
- - [x] Classes 3 / 3
   - - [x]  Class
   - - [x]  Class instances
   - - [x]  Class vs instance <br><br>

- - [ ] Methods 3 / 4
   - - [x]  Methods
   - - [x]  Methods and attributes
   - - [x]  Magic methods
   - - [ ]  Math magic methods <br><br>

- - [ ] Advanced OOP 2 / 5 
   - - [x]  Inheritance
   - - [x]  Method overriding
   - - [ ]  Abstract classes
   - - [ ]  Decorators in OOP
   - - [ ]  Multiple inheritance <br><br>

